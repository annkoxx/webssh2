...
- 支持密码和 SSH 密钥两种认证方式
- 支持 IPv4、IPv6 和域名 SSH 登录，裸 IPv6 会自动转换为标准的 `[IPv6]:端口`
- SOCKS5 代理连接支持
- SFTP 文件管理（浏览/上传/下载）
...
| `你的域名/2603:c021:8012:ef00:0:dd95:ca1:7387/mypass` | root@[2603:c021:8012:ef00:0:dd95:ca1:7387]:22，密码 mypass |
| `你的域名/[2603:c021:8012:ef00:0:dd95:ca1:7387]/admin/mypass` | admin@[2603:c021:8012:ef00:0:dd95:ca1:7387]:22，密码 mypass |
| `你的域名/2603:c021:8012:ef00:0:dd95:ca1:7387/2222/admin/mypass` | admin@[2603:c021:8012:ef00:0:dd95:ca1:7387]:2222，密码 mypass |
| `你的域名/[2603:c021:8012:ef00:0:dd95:ca1:7387]:2222/admin/mypass` | admin@[2603:c021:8012:ef00:0:dd95:ca1:7387]:2222，密码 mypass |
| `你的域名/[2603:c021:8012:ef00:0:dd95:ca1:7387]/22/root/-----BEGIN RSA...` | root@[2603:c021:8012:ef00:0:dd95:ca1:7387]:22，**私钥登录** |

> 自动识别：最后一段若包含 `-----BEGIN` 或长度超过 200 字符，自动切换为私钥认证；否则视为密码。
>
> IPv6 地址带不带方括号都可以识别。自定义端口推荐使用独立路径段 `IPv6/2222/...`；如果把端口和 IPv6 写在同一段，必须使用 `[IPv6]:2222`，避免把端口误认为 IPv6 地址的一部分。
...
未检测到可用 IPv6 时会显示警告并提示按回车继续；检测到 IPv6 时不会显示
```

向导最后会检测宿主机是否具有全局 IPv6 地址和默认 IPv6 路由。没有 IPv6 时会提示该服务器无法直接连接 IPv6 SSH，并等待按回车继续；检测到 IPv6 时保持安静。回答完成后自动 `docker compose up -d --build` 启动。启动成功后浏览器打开 `http://你的服务器IP:8008` 即可。
...
系统会自动识别并在建立 SSH 连接时使用标准的 `[IPv6]:端口` 格式；已经带方括号的地址也可以正常使用。部署 WebSSH 的宿主服务器必须具有可用的 IPv6 网络，否则无法直接连接只有 IPv6 地址的 SSH 服务器。
